plot_map <- function(input_data,
  ...
  #' @param highlight_id_label Region id variable to be used for highlighting on mouse when reactive = T (default is "iso_a3").
  ...
  countries_sf <- rnaturalearth::ne_countries(scale = "large",
                                              returnclass = "sf")
  ...
  if (reactive) {
    ...
    pal <-
      leaflet::colorNumeric(palette = "viridis", domain = countries_merged[[var_value]])
    
    # Create the map object
    plot <- leaflet::leaflet(data = countries_merged)
      plot <- leaflet::setView(plot, lat = 0, lng = 0, zoom = 1) 
      plot <- leaflet::addPolygons(plot, 
        fillColor = ~ pal(countries_merged[[var_value]]),
        fillOpacity = .7,
        color = borders_color,
        weight = borders_size,
        ...
        label = ~ paste0(get(highlight_label), sep = ": ", get(var_value))
      )
      plot
    ...
    plot <-
      leaflet::addLegend(
        ...
        position = legend_pos,
        pal = pal,
        values = ~ countries_merged[[var_value]],
        labFormat = labels,
        opacity = 1,
        title = legend_title
      )
